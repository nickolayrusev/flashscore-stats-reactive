...
import com.jboxers.flashscore.config.RedisMessageListener;
...
public class AppService {
    ...
    @Autowired
    public AppService(ReactiveRedisConnectionFactory factory, FlashScoreService flashScoreService, ObjectMapper objectMapper) {
    ...
    public List<Stat> deserializeStats(String s) {
        try {
    ...
    public Mono<Boolean> fetchTomorrowAndSave() {
        ...
        return this.keyCommands.exists(finalDateKey).flatMap(r -> {
            return !r ? this.flashScoreService.fetchTomorrow().flatMap(l -> saveTomorrowData(serializeValues(l))) : Mono.just(false);
        });
    }
    ...
    public Mono<Boolean> fetchYesterdayAndSave() {
        ...
        return this.keyCommands.exists(finalDateKey).flatMap(r -> {
            return !r ? this.flashScoreService.fetchYesterday().flatMap(l -> saveYesterdayData(serializeValues(l))) : Mono.just(false);
        });
    }
    ...
    public Flux<Boolean> saveStanding() {
        return this.listCommands.lLen(toByteBuffer(STANDINGS))
